...
if ( ! class_exists( 'Pixerex_Templates_Manager' ) ) {
	...
	class Pixerex_Templates_Manager {
		...
		public function __construct() {
            ...
            //Register AJAX hooks
			add_action( 'wp_ajax_pixerex_get_templates', array( $this, 'get_templates' ) );
            add_action( 'wp_ajax_pixerex_inner_template', array( $this, 'insert_inner_template' ) );
			...
			add_filter( 'pixerex-templates-core/assets/editor/localize', array( $this, 'localize_tabs' ) );
		...
		public function register_sources() {
			...
			require PIXEREX_ADDONS_PATH . 'includes/templates/sources/base.php';

            $namespace = str_replace( 'Classes', 'Sources' , __NAMESPACE__ );
            
			$sources = array(
				'pixerex-api'   =>  $namespace . '\Pixerex_Templates_Source_Api',
			);

			foreach ( $sources as $key => $class ) {
                
				require PIXEREX_ADDONS_PATH . 'includes/templates/sources/' . $key . '.php';
		...
		public function get_template_tabs() {
			...
			$tabs = Templates\pixerex_templates()->types->get_types_for_popup();
         ...
         * 
		 */
		public function pixerex_get_template_data() {
