(function ($) {
    /****** Pixerex Vertical Scroll Handler ******/
    var PixerexVerticalScrollHandler = function ($scope, $) {
        ...
        var $vScrollElem = $scope.find(".pixerex-vscroll-wrap"),
            instance = null,
            vScrollSettings = $vScrollElem.data("settings");

        vScrollSettings.deviceType = deviceType;

        instance = new pixerexVerticalScroll($vScrollElem, vScrollSettings);
        instance.init();

    };

    window.pixerexVerticalScroll = function ($selector, settings) {
        var self = this,
            $window = $(window),
            isTouch = false,
            $instance = $selector,
            checkTemps = $selector.find(".pixerex-vscroll-sections-wrap")
                .length,
            $htmlBody = $("html, body"),
            $itemsList = $(".pixerex-vscroll-dot-item", $instance),
            $menuItems = $(".pixerex-vscroll-nav-item", $instance),
            defaultSettings = {
                speed: 700,
                offset: 0,
                fullSection: true
        ...
        self.init = function () {
            ...
            $itemsList.on("click.pixerexVerticalScroll", self.onNavDotChange);
            $menuItems.on("click.pixerexVerticalScroll", self.onNavDotChange);

            $itemsList.on("mouseenter.pixerexVerticalScroll", self.onNavDotEnter);

            $itemsList.on("mouseleave.pixerexVerticalScroll", self.onNavDotLeave);

            if ("desktop" === settings.deviceType) {
                $window.on("scroll.pixerexVerticalScroll", self.onWheel);
            }

            $window.on("resize.pixerexVerticalScroll orientationchange.pixerexVerticalScroll", self.debounce(50, self.onResize));
            ...
            if (settings.animation) {
                $instance.find(".pixerex-vscroll-dots").removeClass("elementor-invisible").addClass("animated " + settings.animation + " animated-" + settings.duration);
            }
        ...
        self.checkCurrentActive = function () {
            ...
            if (currentSection)
                $(".pixerex-vscroll-dots").removeClass("pixerex-vscroll-dots-hide");
        ...
        self.onTouchMove = function (e) {
            ...
            var $target = $(e.target),
                sectionSelector = checkTemps ? ".pixerex-vscroll-temp" : ".elementor-top-section",
            ...
            if (sectionId && sections.hasOwnProperty(sectionId)) {
                ...
                if (newSectionId) {
                    ...
                    $(".pixerex-vscroll-dots, .pixerex-vscroll-nav-menu").removeClass("pixerex-vscroll-dots-hide");
                    ...
                    if ("down" === direction) {
                        ...
                        if ($lastselector.offset().top + $lastselector.innerHeight() - $(document).scrollTop() > 600) {
                            ...
                            $(".pixerex-vscroll-dots, .pixerex-vscroll-nav-menu").addClass("pixerex-vscroll-dots-hide");
        ...
        self.onNavDotEnter = function () {
            ...
            if (settings.tooltips) {
                $(
                    '<div class="pixerex-vscroll-tooltip"><span>' +
        ...
        self.onNavDotLeave = function () {
            $(".pixerex-vscroll-tooltip").fadeOut(200, function () {
                $(this).remove();
            });
        };
        ...
        self.onNavDotChange = function (event) {
            ...
            if (!isScrolling) {
                ...
                if ($this.hasClass("pixerex-vscroll-nav-item")) {
        ...
        self.onKeyUp = function (event, direction) {
            ...
            var direction = direction || "up",
                nextItem = $(".pixerex-vscroll-dot-item[data-menuanchor=" + currentSection + "]", $instance).next(),
                prevItem = $(".pixerex-vscroll-dot-item[data-menuanchor=" + currentSection + "]", $instance).prev();
            ...
            if ("up" === direction) {
                if (prevItem[0]) {
                    prevItem.trigger("click.pixerexVerticalScroll");
                }
            }

            if ("down" === direction) {
                if (nextItem[0]) {
                    nextItem.trigger("click.pixerexVerticalScroll");
                }
            }
        };
        ...
        self.onWheel = function (event) {
            ...
            var $target = $(event.target),
                ...
                windowScrollTop = $window.scrollTop(),
                dotIndex = $(".pixerex-vscroll-dot-item.active").index();
            ...
            if (isTouch) {
                ...
                if (dotIndex === $itemsList.length - 1 && !$vTarget) {
                    ...
                    if (
                        ...
                        $(
                            ".pixerex-vscroll-dots, .pixerex-vscroll-nav-menu"
                        ).addClass("pixerex-vscroll-dots-hide");
                    }
                } else {
                    $(
                        ".pixerex-vscroll-dots, .pixerex-vscroll-nav-menu"
                    ).removeClass("pixerex-vscroll-dots-hide");
                }
            }
    ...
    $(window).on("elementor/frontend/init", function () {
        elementorFrontend.hooks.addAction(
            "frontend/element_ready/pixerex-vscroll.default",
            PixerexVerticalScrollHandler
        );
    });
})(jQuery);